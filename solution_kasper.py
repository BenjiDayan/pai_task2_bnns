...
def run_solution(dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, output_dir: str = '/results/',
    ...
    if model is None:
        # Create model
        model = Model(use_densenet=False, num_epochs=100)
...
class Model(object):
    ...
    def __init__(self, num_epochs=100, batch_size=128, learning_rate=1e-3, hidden_layers=(100,100), use_densenet=False, print_interval=100):
    ...
    def train(self, dataset: torch.utils.data.Dataset):
        ...
        has_reached_90 = False
        good_time_to_stop = False
        for _ in progress_bar:
            num_batches = len(train_loader)
            pi_i = 2**(num_batches - 1) / (2**num_batches - 1)
            full_warmup_epoch = 5
            lambda_regularization = min(full_warmup_epoch, current_epoch_number) / full_warmup_epoch
            current_epoch_number += 1
            nll_loss_running = 0.
            log_var_post_running = 0.
            log_prior_running = 0.
            reg_term_running = 0.
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    current_logits, log_prior, log_var_post = self.network.forward(batch_x)
                    #print(current_logits, log_prior, log_var_post)
                    nll_loss = F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum')
                    reg_term = (log_var_post - log_prior)
                    loss = nll_loss + lambda_regularization * reg_term
                    loss.backward()

                    nll_loss_running += nll_loss.item()
                    reg_term_running += reg_term.item()
                    log_prior_running += log_prior.item()
                    log_var_post_running += log_var_post.item()
                ...
                if batch_idx % self.print_interval == 0:
                    ...
                    progress_bar.set_postfix(loss=loss.item(), acc=current_accuracy.item(), lam=lambda_regularization)
                    if(current_accuracy.item() > .9):
                        has_reached_90 = True
                    if(has_reached_90 and current_accuracy.item() < 0.7):
                        print("Now would be good to stop")
                        good_time_to_stop = True
                        break
            print('Running', nll_loss_running, reg_term_running, log_prior_running, log_var_post_running)
...
class BayesianLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.use_bias:
            ...
            #print('bias log prob', bias_log_prob.shape)
            #print('log prior shape', log_prior.shape)
...
class UnivariateGaussian(ParameterDistribution):
    ...
    def sample(self) -> torch.Tensor:
        ...
        #return self.mu  #self.dist.sample()
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        #self.dist = normal # torch.distributions.Independent(normal, 1)

    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        # TODO: Implement this
        my_dist = torch.distributions.Normal(self.mu, torch.exp(self.rho))
        log_prob = my_dist.log_prob(values)
        #print('logprob shape', log_prob.shape)
        return log_prob.sum()

    def sample(self) -> torch.Tensor:
        # TODO: Implement this
        # raise NotImplementedError()
        return self.mu + torch.randn_like(self.rho) * torch.exp(self.rho) #self.dist.sample()
